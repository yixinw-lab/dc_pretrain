...
wandb_project="${WANDB_PROJECT:-overtrain-dclm-400m}"
wandb_group_prefix="${WANDB_GROUP_PREFIX:-baseline-adam-scalinglaw-400m-selected}"
wandb_mode="${WANDB_MODE:-offline}"
warmup_ratio="${WARMUP_RATIO:-0.01}"
max_grad_norm="${MAX_GRAD_NORM:-1.0}"

# Selected baseline configs from exps_results.md for DCLM 400M:
# model_tag|n_layer|n_head|n_kv_head|n_embd|device_batch_size|num_epochs|weight_decay|lr
configs=(
  "1p4b|24|32|32|2048|16|32|0.8|1e-3"
  "664m|18|24|24|1536|16|32|0.8|1e-3"
  "257m|12|16|16|1024|16|32|0.4|1e-3"
...
echo "Launching ${total_jobs} selected baseline 400m runs sequentially."
...
for config in "${configs[@]}"; do
  ...
  run_base_name="text_dclm400m_baseline_adam_${model_tag}_e${num_epochs}_wd${wd_tag}_lr${lr_tag}"
  ...
  WANDB_MODE="$wandb_mode" PYTHONUNBUFFERED=1 \
    torchrun --standalone --nproc_per_node="$nproc_per_node" train4_diffusionauxiliary.py \
      ...
      --log-grad-norms \
      --input_bin "dclm_data_400m/dclm_train.pt" \
      --input_val_bin "dclm_data_400m/dclm_val.pt" \
...
echo
echo "All selected baseline 400m runs finished."
