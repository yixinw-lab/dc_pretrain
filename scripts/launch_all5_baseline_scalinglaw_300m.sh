...
wandb_group_prefix="${WANDB_GROUP_PREFIX:-baseline-adam-scalinglaw-300m-selected}"
wandb_mode="${WANDB_MODE:-offline}"
warmup_ratio="${WARMUP_RATIO:-0.01}"
max_grad_norm="${MAX_GRAD_NORM:-1.0}"

# Selected baseline configs from exps_results.md for DCLM 300M:
# model_tag|n_layer|n_head|n_kv_head|n_embd|device_batch_size|num_epochs|weight_decay|lr
configs=(
  "1p4b|24|32|32|2048|16|32|1.6|1e-3"
...
echo "Launching ${total_jobs} selected baseline 300m runs sequentially."
...
for config in "${configs[@]}"; do
  ...
  run_base_name="text_dclm300m_baseline_adam_${model_tag}_e${num_epochs}_wd${wd_tag}_lr${lr_tag}"
...
echo
echo "All selected baseline 300m runs finished."
