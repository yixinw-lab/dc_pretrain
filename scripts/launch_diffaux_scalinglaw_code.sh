...
source /home/ubuntu/slowrun-pri/.venv/bin/activate
...
wandb_project="${WANDB_PROJECT:-overtrain-stackv2}"
...
for config in "${configs[@]}"; do
  ...
  run_base_name="code_stackv2_diffaux_${model_tag}_scalinglaw_e${num_epochs}_wd${wd_tag}_lr${lr_tag}_finaleval_${final_eval_size}"
  ...
  WANDB_MODE="$wandb_mode" PYTHONUNBUFFERED=1 \
    torchrun --standalone --nproc_per_node="$nproc_per_node" train4_diffusionauxiliary.py \
      ...
      --log-grad-norms \
      --input_bin "prepared_data/stackv2/stackv2_train.pt" \
      --input_val_bin "prepared_data/stackv2/stackv2_val.pt" \
