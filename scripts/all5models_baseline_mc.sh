...
wandb_group_prefix="${WANDB_GROUP_PREFIX:-baseline-adam-scalinglaw}"
wandb_mode="${WANDB_MODE:-offline}"
warmup_ratio="${WARMUP_RATIO:-0.01}"
max_grad_norm="${MAX_GRAD_NORM:-1.0}"
seed_values="${MC_SEEDS:-1 2 3 4}"

read -r -a seeds <<< "$seed_values"

configs=(
  "72m|6|8|8|512|16|32|0.4|1e-2"
  "140m|9|12|12|768|16|32|0.8|3e-3"
  "257m|12|16|16|1024|16|16|1.6|1e-3"
  "664m|18|24|24|1536|16|16|1.6|1e-3"
  "1p4b|24|32|32|2048|8|16|3.2|1e-3"
)

total_jobs=$(( ${#configs[@]} * ${#seeds[@]} ))
current_job=0
...
echo "Launching ${total_jobs} baseline scaling-law runs sequentially."
echo "mc_seeds=${seeds[*]}"
...
for config in "${configs[@]}"; do
  ...
  for seed in "${seeds[@]}"; do
    ...
    run_base_name="text_dclm_${model_tag}_scalinglaw_seed${seed}_e${num_epochs}_wd${wd_tag}_lr${lr_tag}_finaleval_${final_eval_size}"
    ...
    echo "  device_batch_size=$device_batch_size epochs=$num_epochs weight_decay=$weight_decay adam_lr=$lr seed=$seed"
    echo "  warmup_ratio=$warmup_ratio max_grad_norm=$max_grad_norm"

    set +e
    WANDB_MODE="$wandb_mode" PYTHONUNBUFFERED=1 \
    torchrun --standalone --nproc_per_node="$nproc_per_node" train4_diffusionauxiliary_mc.py \
      --run "$run_name" \
      --seed "$seed" \
      --n_layer "$n_layer" \
      --n_head "$n_head" \
      ...
      --log-grad-norms \
      --input_bin "dclm_data/dclm_train.pt" \
      --input_val_bin "dclm_data/dclm_val.pt" \
      --num-epochs "$num_epochs" \
      --adaptive-stage0-epochs "$num_epochs" \
...
echo
echo "All baseline scaling-law runs finished."
